...
MUTATIONS = [
    ...
    ("every genre borrows the same studio colour", "synth/prompting.py",
     "        pool = list(dict.fromkeys(genre.texture))",
     "        pool = list(dict.fromkeys(SPACE))"),
    ("instrument line never changes", "synth/prompting.py",
     "        chosen_instruments = rng.choice(genre.instruments)",
     "        chosen_instruments = genre.instruments[0]"),
