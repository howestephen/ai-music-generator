...
| 2.6 | Honest render estimate and exclusive playback | `done` | The first render of a backend pays for its weight download; counting that as render time made a 40-second job report an hour and read as hung. Estimates now use the most recent renders only, with per-backend fallbacks, and a running card shows elapsed seconds. Starting one history track now stops any other. Reported by owner 2026-09-19 | none | 2.5 |
