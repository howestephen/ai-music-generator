...
| 0.3.2 | Model-derived UI controls | `planned` | The Gradio input schema still imposes ACE-Step's 240-second range on MiniMax, even though duration caps belong to each backend. Build the selected model's duration range, defaults and supported settings from the backend registry; hide or disable unsupported controls instead of sending irrelevant shared parameters. API submissions and browser submissions must enforce the same selected-model contract. This surfaced when a 270-second MiniMax request was rejected by the UI before reaching the backend | none | 0.3.1 |
...
- Current milestone: 0.3.2, make every UI control and validation rule derive from the
  selected backend
- Then: finish 0.4; 0.5 and 0.6 follow in either order
