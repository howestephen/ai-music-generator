...
class ServedUi(unittest.TestCase):
    ...
    def test_the_page_is_react_and_puts_regenerate_beside_the_prompt(self):
        ...
        self.assertIn(">Remix<", source)
        self.assertLess(source.index(">Generate<"), source.index(">Remix<"))
        self.assertNotIn("<summary>Remix", source)
        self.assertIn("advanced-disclosure", source)
        advanced = css.split("#controls-panel .advanced-disclosure summary", 1)[1].split("}", 1)[0]
        self.assertIn("font-size: 1rem", advanced)
        self.assertIn("min-height: 2.5rem", advanced)
